 ...
 * 
 */
public class SbAcquireStepPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public SbAcquireStepPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject sbAcquireStep, String editing_mode) {
		super(editingContext, sbAcquireStep, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = SoftwareBehaviorViewsRepository.class;
		partKey = SoftwareBehaviorViewsRepository.SbAcquireStep.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final SbAcquireStep sbAcquireStep = (SbAcquireStep)elt;
			final SbAcquireStepPropertiesEditionPart basePart = (SbAcquireStepPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, sbAcquireStep.getName()));
			
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.inputStepRelation)) {
				inputStepRelationSettings = new ReferencesTableSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation());
				basePart.initInputStepRelation(inputStepRelationSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.outputStepRelation)) {
				outputStepRelationSettings = new ReferencesTableSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation());
				basePart.initOutputStepRelation(outputStepRelationSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.representedOperation)) {
				// init part
				representedOperationSettings = new EObjectFlatComboSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation());
			...
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.interactionResource)) {
				// init part
				interactionResourceSettings = new EObjectFlatComboSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource());
			...
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpExecutionTime)) {
				// init part
				rtpExecutionTimeSettings = new EObjectFlatComboSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime());
				basePart.initRtpExecutionTime(rtpExecutionTimeSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpDeadline)) {
				// init part
				rtpDeadlineSettings = new EObjectFlatComboSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline());
				basePart.initRtpDeadline(rtpDeadlineSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpBlockingTime)) {
				// init part
				rtpBlockingTimeSettings = new EObjectFlatComboSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime());
				basePart.initRtpBlockingTime(rtpBlockingTimeSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpRepetition)) {
				// init part
				rtpRepetitionSettings = new EObjectFlatComboSettings(sbAcquireStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpRepetition());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.name) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_Name();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.inputStepRelation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.outputStepRelation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.representedOperation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.interactionResource) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpExecutionTime) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpDeadline) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpBlockingTime) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpRepetition) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		SbAcquireStep sbAcquireStep = (SbAcquireStep)semanticObject;
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.name == event.getAffectedEditor()) {
			sbAcquireStep.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.inputStepRelation == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.outputStepRelation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof SbStepPrecedenceRelation) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.representedOperation == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.interactionResource == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpExecutionTime == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpDeadline == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpBlockingTime == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
		...
		if (SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpRepetition == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			SbAcquireStepPropertiesEditionPart basePart = (SbAcquireStepPropertiesEditionPart)editingPart;
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_Name().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.name)) {
			...
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation().equals(msg.getFeature())  && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.inputStepRelation))
				basePart.updateInputStepRelation();
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation().equals(msg.getFeature())  && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.outputStepRelation))
				basePart.updateOutputStepRelation();
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.representedOperation))
				basePart.setRepresentedOperation((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.interactionResource))
				basePart.setInteractionResource((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpExecutionTime))
				basePart.setRtpExecutionTime((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpDeadline))
				basePart.setRtpDeadline((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpBlockingTime))
				basePart.setRtpBlockingTime((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpRepetition().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.rtpRepetition))
	...
	public boolean isRequired(Object key, int kind) {
		return key == SoftwareBehaviorViewsRepository.SbAcquireStep.Properties.name;
	}
