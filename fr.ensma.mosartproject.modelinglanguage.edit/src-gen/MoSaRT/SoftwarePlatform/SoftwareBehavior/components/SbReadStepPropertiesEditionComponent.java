 ...
 * 
 */
public class SbReadStepPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public SbReadStepPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject sbReadStep, String editing_mode) {
		super(editingContext, sbReadStep, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = SoftwareBehaviorViewsRepository.class;
		partKey = SoftwareBehaviorViewsRepository.SbReadStep.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final SbReadStep sbReadStep = (SbReadStep)elt;
			final SbReadStepPropertiesEditionPart basePart = (SbReadStepPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, sbReadStep.getName()));
			
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.inputStepRelation)) {
				inputStepRelationSettings = new ReferencesTableSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation());
				basePart.initInputStepRelation(inputStepRelationSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.outputStepRelation)) {
				outputStepRelationSettings = new ReferencesTableSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation());
				basePart.initOutputStepRelation(outputStepRelationSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.representedOperation)) {
				// init part
				representedOperationSettings = new EObjectFlatComboSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation());
			...
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.interactionResource)) {
				// init part
				interactionResourceSettings = new EObjectFlatComboSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource());
			...
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpExecutionTime)) {
				// init part
				rtpExecutionTimeSettings = new EObjectFlatComboSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime());
				basePart.initRtpExecutionTime(rtpExecutionTimeSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpDeadline)) {
				// init part
				rtpDeadlineSettings = new EObjectFlatComboSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline());
				basePart.initRtpDeadline(rtpDeadlineSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpBlockingTime)) {
				// init part
				rtpBlockingTimeSettings = new EObjectFlatComboSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime());
				basePart.initRtpBlockingTime(rtpBlockingTimeSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpRepetition)) {
				// init part
				rtpRepetitionSettings = new EObjectFlatComboSettings(sbReadStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpRepetition());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.name) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_Name();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.inputStepRelation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.outputStepRelation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.representedOperation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.interactionResource) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpExecutionTime) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpDeadline) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpBlockingTime) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpRepetition) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		SbReadStep sbReadStep = (SbReadStep)semanticObject;
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.name == event.getAffectedEditor()) {
			sbReadStep.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.inputStepRelation == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.outputStepRelation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof SbStepPrecedenceRelation) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.representedOperation == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.interactionResource == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpExecutionTime == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpDeadline == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpBlockingTime == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
		...
		if (SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpRepetition == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			SbReadStepPropertiesEditionPart basePart = (SbReadStepPropertiesEditionPart)editingPart;
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_Name().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.name)) {
			...
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation().equals(msg.getFeature())  && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.inputStepRelation))
				basePart.updateInputStepRelation();
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation().equals(msg.getFeature())  && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.outputStepRelation))
				basePart.updateOutputStepRelation();
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.representedOperation))
				basePart.setRepresentedOperation((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.interactionResource))
				basePart.setInteractionResource((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpExecutionTime))
				basePart.setRtpExecutionTime((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpDeadline))
				basePart.setRtpDeadline((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpBlockingTime))
				basePart.setRtpBlockingTime((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpRepetition().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbReadStep.Properties.rtpRepetition))
	...
	public boolean isRequired(Object key, int kind) {
		return key == SoftwareBehaviorViewsRepository.SbReadStep.Properties.name;
	}
