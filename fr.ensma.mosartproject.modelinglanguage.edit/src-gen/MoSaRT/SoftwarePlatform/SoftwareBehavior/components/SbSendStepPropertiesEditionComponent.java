 ...
 * 
 */
public class SbSendStepPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public SbSendStepPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject sbSendStep, String editing_mode) {
		super(editingContext, sbSendStep, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = SoftwareBehaviorViewsRepository.class;
		partKey = SoftwareBehaviorViewsRepository.SbSendStep.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final SbSendStep sbSendStep = (SbSendStep)elt;
			final SbSendStepPropertiesEditionPart basePart = (SbSendStepPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.name))
				basePart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, sbSendStep.getName()));
			
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.inputStepRelation)) {
				inputStepRelationSettings = new ReferencesTableSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation());
				basePart.initInputStepRelation(inputStepRelationSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.outputStepRelation)) {
				outputStepRelationSettings = new ReferencesTableSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation());
				basePart.initOutputStepRelation(outputStepRelationSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.representedOperation)) {
				// init part
				representedOperationSettings = new EObjectFlatComboSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation());
			...
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.interactionResource)) {
				// init part
				interactionResourceSettings = new EObjectFlatComboSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource());
			...
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpExecutionTime)) {
				// init part
				rtpExecutionTimeSettings = new EObjectFlatComboSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime());
				basePart.initRtpExecutionTime(rtpExecutionTimeSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpDeadline)) {
				// init part
				rtpDeadlineSettings = new EObjectFlatComboSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline());
				basePart.initRtpDeadline(rtpDeadlineSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpBlockingTime)) {
				// init part
				rtpBlockingTimeSettings = new EObjectFlatComboSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime());
				basePart.initRtpBlockingTime(rtpBlockingTimeSettings);
			}
			if (isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpRepetition)) {
				// init part
				rtpRepetitionSettings = new EObjectFlatComboSettings(sbSendStep, SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpRepetition());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.name) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_Name();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.inputStepRelation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.outputStepRelation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.representedOperation) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.interactionResource) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpExecutionTime) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpDeadline) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpBlockingTime) {
			return SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime();
		}
		if (editorKey == SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpRepetition) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		SbSendStep sbSendStep = (SbSendStep)semanticObject;
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.name == event.getAffectedEditor()) {
			sbSendStep.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.inputStepRelation == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.outputStepRelation == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof SbStepPrecedenceRelation) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.representedOperation == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.interactionResource == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpExecutionTime == event.getAffectedEditor()) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpDeadline == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpBlockingTime == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
		...
		if (SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpRepetition == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.EDIT) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			SbSendStepPropertiesEditionPart basePart = (SbSendStepPropertiesEditionPart)editingPart;
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_Name().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.name)) {
			...
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_InputStepRelation().equals(msg.getFeature())  && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.inputStepRelation))
				basePart.updateInputStepRelation();
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_OutputStepRelation().equals(msg.getFeature())  && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.outputStepRelation))
				basePart.updateOutputStepRelation();
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RepresentedOperation().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.representedOperation))
				basePart.setRepresentedOperation((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_InteractionResource().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.interactionResource))
				basePart.setInteractionResource((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpExecutionTime().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpExecutionTime))
				basePart.setRtpExecutionTime((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpDeadline().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpDeadline))
				basePart.setRtpDeadline((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpBlockingTime().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpBlockingTime))
				basePart.setRtpBlockingTime((EObject)msg.getNewValue());
			if (SoftwareBehaviorPackage.eINSTANCE.getSbStep_RtpRepetition().equals(msg.getFeature()) && basePart != null && isAccessible(SoftwareBehaviorViewsRepository.SbSendStep.Properties.rtpRepetition))
	...
	public boolean isRequired(Object key, int kind) {
		return key == SoftwareBehaviorViewsRepository.SbSendStep.Properties.name;
	}
